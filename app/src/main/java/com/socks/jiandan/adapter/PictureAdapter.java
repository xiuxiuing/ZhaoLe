...
public class PictureAdapter extends RecyclerView.Adapter<PictureAdapter.PictureViewHolder> {
    ...
    public void onBindViewHolder(final PictureViewHolder holder, final int position) {
        ...
        ImageLoadProxy.displayImageList(picUrl, holder.img, R.drawable.ic_loading_large, new SimpleImageLoadingListener() {
            @Override
        ...
        holder.img_share.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new MaterialDialog.Builder(mActivity).items(R.array.picture_dialog)
                        .backgroundColor(mActivity.getResources().getColor(JDApplication.COLOR_OF_DIALOG))
                        .contentColor(JDApplication.COLOR_OF_DIALOG_CONTENT).itemsCallback(new MaterialDialog.ListCallback() {
                            @Override
    ...
    private void getCommentCounts(final ArrayList<Picture> pictures) {
        ...
        RequestManager.addRequest(
                new Request4CommentCounts(CommentNumber.getCommentCountsURL(sb.toString()), new Response.Listener<ArrayList<CommentNumber>>() {
                    ...
                    public void onResponse(ArrayList<CommentNumber> response) {
                        ...
                        for (int i = 0; i < pictures.size(); i++) {
                            pictures.get(i).setComment_counts("");
                        }
                        if (page == 1) {
                        ...
                        notifyDataSetChanged();
                        // 加载完毕后缓存
                        PictureCache.getInstance(mActivity).addResultCache(JSONParser.toString(pictures), page);
                    }
                }, new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
